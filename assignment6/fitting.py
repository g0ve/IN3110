...
def fit(filename, feature):
    ...
    Y_train = training_set[feature].values #Bare feature
    X_train = training_set.drop(columns=[feature]) #Hele

    Y_valid = validation_set[feature].values #Feature
    X_valid = validation_set.drop(columns=[feature]) #Hele



    # print(df)
    # x1 = df.drop(columns=[feature])
    # y1 = df[feature].values
    #
    #
